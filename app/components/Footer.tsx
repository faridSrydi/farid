...
export default function Footer() {
  ...
  return (
    <footer className="relative bg-[#030014] pt-20 pb-10 border-t border-white/5 overflow-hidden">
      {/* Decorative "Laser" Top Border */}
      <div className="absolute top-0 left-1/2 -translate-x-1/2 w-3/4 h-[1px] bg-gradient-to-r from-transparent via-purple-500/50 to-transparent blur-sm"></div>
            ...
            <ul className="space-y-4">
              {footerLinks.map((link, idx) => (
                <li key={idx}>
                  <Link
                    href={link.href}
                    className="text-sm text-gray-500 hover:text-purple-400 transition-colors duration-300 flex items-center gap-2 group"
                  >
            ...
            <div className="flex gap-4">
              {socialLinks.map((link, idx) => (
                <motion.a
                  ...
                  whileTap={{ scale: 0.95 }}
                  className="w-10 h-10 rounded-full bg-white/5 border border-white/10 flex items-center justify-center text-gray-400 hover:text-white hover:bg-white/10 hover:border-purple-500/50 hover:shadow-[0_0_15px_-5px_rgba(168,85,247,0.5)] transition-all duration-300"
